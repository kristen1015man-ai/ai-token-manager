...
生产备份与恢复流程见 [docs/backup-restore.md](docs/backup-restore.md)。上线前必须完成一次恢复演练。
...
Use the Docker dual-service path for production. The public entrypoint is nginx
on 80/443; web:3000 and proxy:3001 are bound to 127.0.0.1 for local diagnostics only.

1. Copy .env.production.example to .env and fill all required secrets.
2. Put tls.crt and tls.key in nginx/ssl/.
3. Start with: docker compose --profile edge up -d --build

Production URLs:
- Admin dashboard: https://ai.yourcompany.com
- Employee API proxy: https://ai.yourcompany.com/v1
