...
export async function GET(request: NextRequest) {
  ...
  const feishuError = searchParams.get("error") || searchParams.get("error_code");
  const feishuErrorDescription = searchParams.get("error_description") || searchParams.get("error_msg") || searchParams.get("message");

  if (!code) {
    console.warn("[Feishu OAuth] callback without code", {
      error: feishuError || "missing_code",
      hasDescription: Boolean(feishuErrorDescription),
    });
    const host = request.headers.get("host") || "ai.seapllo.com";
    const protocol = request.headers.get("x-forwarded-proto") || "https";
    const loginError = feishuError ? "feishu_denied" : "no_code";
    return NextResponse.redirect(new URL(`/login?error=${loginError}`, `${protocol}://${host}`));
  }
  ...
  try {
    ...
    console.log("[Feishu OAuth] user info received", {
      hasOpenId: Boolean(userInfo.open_id),
      hasName: Boolean(userInfo.name),
      hasEmployeeNo: Boolean(userInfo.employee_no),
    });
