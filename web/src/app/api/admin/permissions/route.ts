...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  const { db } = await getDb();
  const rows = await db.select({ id: users.id, role: users.role, status: users.status }).from(users).where(eq(users.id, userId)).limit(1);
  if (rows.length === 0) {
    return NextResponse.json({ error: "用户不存在" }, { status: 404 });
  }
  if (rows[0].status !== "active") {
    return NextResponse.json({ error: "用户已停用" }, { status: 400 });
  }
...
export const PUT = apiHandler(async (request: NextRequest) => {
  ...
  const { db } = await getDb();
  const targetRows = await db.select({ role: users.role, status: users.status }).from(users).where(eq(users.id, userId)).limit(1);
