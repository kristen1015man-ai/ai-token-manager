...
export async function fetchDepartmentUsersRaw(appToken: string, departmentId: string) {
  ...
  do {
    ...
    if (!resp.ok) {
      throw new Error(`Fetch department users failed: department=${departmentId}, HTTP ${resp.status}`);
    }
    const data = await resp.json();
    if (data.code !== 0) {
      throw new Error(`Fetch department users failed: department=${departmentId}, code=${data.code}, msg=${data.msg || data.message || ""}`);
    }
    allUsers.push(...(data.data?.items || []));
    pageToken = data.data?.has_more ? data.data?.page_token : "";
    pageCount += 1;
    if (pageCount > 200) {
      throw new Error(`Fetch department users exceeded pagination guard: department=${departmentId}`);
    }
  } while (pageToken);
  return allUsers;
}
