...
function PieTooltip({ active, payload }: { active?: boolean; payload?: Array<{ name: string; value: number }> }) {
  ...
  const d = payload[0];
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-4 text-sm">
              ...
              <tbody>
                {models.map((m, i) => (
                  <tr key={i}>
                    ...
                    <td className="text-right tabular-nums">{m.tokens.toLocaleString()}</td>
                    <td className="text-right tabular-nums">¥{Number(m.cost).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  ...
                  <td className="text-right tabular-nums">{models.reduce((s, m) => s + m.count, 0)}</td>
                  <td className="text-right tabular-nums">{totalTokens.toLocaleString()}</td>
                  <td className="text-right font-semibold tabular-nums">¥{totalCost.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          )}
