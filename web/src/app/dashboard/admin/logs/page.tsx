      ...
      <div className="glass-card-static p-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <GlassSelect
            ...
            options={[
              { value: "", label: "全部操作" },
              ...actionOptions.map(([key, { label }]) => ({ value: key, label })),
            ]}
            placeholder="全部操作"
          ...
          <GlassSelect
            ...
            options={[
              { value: "", label: "全部对象" },
              ...targetOptions.map(([key, { label }]) => ({ value: key, label })),
            ]}
            placeholder="全部对象"
            className="text-sm [&>.glass-select-trigger]:!py-1.5 [&>.glass-select-trigger]:!px-3"
          />
          ...
          <>
            <div className="overflow-x-auto">
              <table className="glass-table">
                <thead>
                  <tr>
                    <th className="text-left w-44">时间</th>
                    <th className="text-left w-24">操作</th>
                    <th className="text-left w-28">对象类型</th>
                    <th className="text-left">对象 ID</th>
                    <th className="text-left w-20">详情</th>
                  </tr>
                </thead>
                <tbody>
                  {logs.map((log) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="font-mono text-xs max-w-[200px] truncate" title={log.targetId}>
                          {log.targetId}
                        </td>
                        <td>
                          <button
