...
export default function AdminOverviewPage() {
  ...
  useEffect(() => {
    ...
    fetchApi<Overview & { channels?: ChannelData[]; models?: ModelData[] }>(`/api/admin/overview?range=${range}&includeBreakdowns=true`)
      .then((d) => {
        if (cancelled) return;
        setData(d);
        setChannelData(d?.channels || []);
        setModelData(d?.models || []);
        setLoading(false);

        fetchApi<{ balanceSummary: BalanceSummary | null }>("/api/admin/overview?onlyBalance=true")
          .then((balanceData) => {
            if (!cancelled) setBalanceSummary(balanceData.balanceSummary);
          })
          .catch(() => {
  ...
  if (!data) {
    return (
      <div className="glass-card-static p-6">
        <h3 className="font-semibold text-gray-900 mb-2">全局概览加载失败</h3>
        <p className="text-sm text-gray-500">{error || "请求未返回有效数据"}</p>
        <button
          onClick={() => setReloadKey((key) => key + 1)}
          className="glass-btn text-sm mt-4"
        >
          重试
        </button>
      </div>
              ...
              <tbody>
                {channelData.map((c, i) => (
                  <tr key={c.channelId}>
                    ...
                    <td className="text-right tabular-nums">{fmt(c.tokens)}</td>
          ...
          <table className="glass-table">
            ...
            <tbody>
              {modelData.map((m, i) => (
                <tr key={m.model}>
                  ...
                  <td className="text-right tabular-nums">{fmt(m.tokens)}</td>
                  <td className="text-right tabular-nums">¥{m.cost.toFixed(2)}</td>
                  <td className="text-right tabular-nums">
            ...
            <tfoot>
              <tr>
                ...
                <td className="text-right tabular-nums">{fmt(modelData.reduce((s, m) => s + m.tokens, 0))}</td>
                <td className="text-right font-semibold tabular-nums">¥{modelTotalCost.toFixed(2)}</td>
                <td className="text-right tabular-nums">100%</td>
                <td className="text-right tabular-nums">{modelData.reduce((s, m) => s + m.count, 0)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
