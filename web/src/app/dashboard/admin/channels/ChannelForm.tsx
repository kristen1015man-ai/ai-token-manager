  ...
  return (
    <div className="glass-card-static p-6 space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div>
          <label className="text-sm text-gray-600 block mb-1">余额模式</label>
          <GlassSelect
            value={form.balanceSyncMode}
            onChange={(v) => setForm({ ...form, balanceSyncMode: v })}
            options={[
              { value: "", label: "按供应商默认" },
        ...
        <div>
          <label className="text-sm text-gray-600 block mb-1">余额预警阈值</label>
          <input
            type="number"
            min={0}
            value={form.balanceAlertThreshold}
            onChange={(e) => setForm({ ...form, balanceAlertThreshold: e.target.value })}
            placeholder="默认 CNY 100 / USD 10"
            className="glass-input w-full"
          />
          <p className="text-xs text-gray-400 mt-1">留空使用币种默认值</p>
        </div>
      </div>
