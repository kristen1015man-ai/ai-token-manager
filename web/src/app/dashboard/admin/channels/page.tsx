...
export default function ChannelsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const parsed = JSON.parse(form.models);
      if (!Array.isArray(parsed) || !parsed.every((m) => typeof m === "string" && m.trim())) {
        showToast("模型列表必须是 JSON 字符串数组");
        return;
      }
      modelsArr = parsed;
    } catch {
      showToast("模型列表必须是有效 JSON");
      return;
    }
  ...
  const startEdit = (ch: Channel) => {
    ...
    setForm({
      ...
      balanceSyncMode: ch.balanceSyncMode || "",
      balanceAlertThreshold: ch.balanceAlertThreshold == null ? "" : String(ch.balanceAlertThreshold),
  ...
  const handleSyncAll = async () => {
    setSyncing(true);
    try {
      const result = await fetchApi<{ synced?: number; failed?: number; alerts?: unknown[]; errors?: Array<{ channelName: string; message: string }> }>("/api/admin/channels/balance-sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if ((result.synced ?? 0) > 0 || (result.failed ?? 0) > 0) {
        const errorText = result.errors?.length
          ? `\n\n失败原因：\n${result.errors.map((e) => `- ${e.channelName}: ${e.message}`).join("\n")}`
          : "";
        alert(`同步完成：成功 ${result.synced ?? 0} 个，失败 ${result.failed ?? 0} 个` + (result.alerts?.length ? `\n⚠️ ${result.alerts.length} 个渠道余额低于阈值` : "") + errorText);
      }
      load();
    } catch (e) {
  ...
  const handleSyncOne = async (ch: Channel) => {
    ...
    try {
      const result = await fetchApi<{ synced?: number; failed?: number; alerts?: unknown[]; errors?: Array<{ channelName: string; message: string }> }>("/api/admin/channels/balance-sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ channelId: ch.id, notify: false }),
      });
      if ((result.failed ?? 0) > 0) {
        const reason = result.errors?.[0]?.message ? `：${result.errors[0].message}` : "";
        alert(`${ch.name} 余额同步失败${reason}`);
      }
      load();
    } catch (e) {
            ...
            <table className="glass-table">
              ...
              <tbody>
                {channels.map((ch) => {
                  ...
                  const keyNeedsReentryByStatus = ch.apiKeyStatus && ch.apiKeyStatus !== "ok";
                  const keyNeedsReentry = typeof ch.apiKey === "string" && ch.apiKey.includes("无法解密");

                  return (
                    <tr key={ch.id}>
                      <td className="whitespace-nowrap">
                        <div>{ch.name}</div>
                        {(keyNeedsReentryByStatus || keyNeedsReentry) && (
                          <div className="mt-1 text-[10px] text-red-500">{ch.apiKeyWarning || "密钥需重新录入"}</div>
                        )}
                      </td>
                      <td className="text-xs font-mono max-w-[200px] truncate">{ch.baseUrl}</td>
                      <td>
                        <div className="flex flex-wrap gap-1 max-w-[250px]">
                          {(Array.isArray(ch.models) ? ch.models : JSON.parse(ch.models || "[]")).map((m: string) => (
                            <span key={m} className="glass-badge glass-badge-indigo !text-[10px] !px-1.5">{m}</span>
                          ))}
                        </div>
                      </td>
                        ...
                        {balanceDisplay ? (
                          <div className="flex flex-col items-center gap-0.5">
                            ...
                            {balanceStatus === "danger" && (
                              <span className="px-1.5 py-0 rounded-full text-[10px] bg-red-50 text-red-500">⚠ 余额告急</span>
                            )}
                      ...
                      <td className="text-center">{ch.priority}</td>
                      <td className="text-center">
