      ...
      <div className="overflow-x-auto">
        <table className="glass-table">
          ...
          <tbody>
            {modelData.map((m, i) => (
              <tr key={m.model}>
                ...
                <td className="text-right">{fmt(m.tokens)}</td>
          ...
          <tfoot>
            <tr>
              <td>合计</td>
              <td className="text-right">{fmt(modelData.reduce((s, m) => s + m.tokens, 0))}</td>
              <td className="text-right">¥{totalCost.toFixed(4)}</td>
              <td className="text-right">100%</td>
              <td className="text-right">{modelData.reduce((s, m) => s + m.count, 0)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
