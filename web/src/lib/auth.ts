...
if (isUnsafeSecret && !isNextBuildPhase && !allowInsecureDevAuth) {
  throw new Error(
    "[FATAL] JWT_SECRET 未配置或使用了不安全的默认值。请设置强随机密钥；仅本地开发可显式设置 ALLOW_INSECURE_DEV_AUTH=true。"
  );
}
