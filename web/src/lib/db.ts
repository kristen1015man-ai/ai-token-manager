...
function flushToDisk(): void {
  ...
  saveInProgress = true;
  try {
    const bytes = Buffer.from(sqliteInstance.export());
    const nextHash = hashBuffer(bytes);
    if (nextHash === lastExportHash) {
      return;
    }
...
export async function getDb() {
  ...
  lastExportHash = buffer ? hashBuffer(buffer) : hashBuffer(Buffer.from(sqlite.export()));
