  ...
  try {
    ...
    const tryExtract = (model: string, displayName: string, searchTerms: string[]): void => {
      for (const term of searchTerms) {
        ...
        if (nums.length >= 2) {
          prices.push(makeUSD(model, displayName, nums[0], nums[1], nums.length >= 3 ? nums[2] : nums[0] * 0.1, rate, "official"));
          return;
        }
      }
    };
  ...
  try {
    ...
    const tryExtract = (model: string, displayName: string, searchTerms: string[]): void => {
      for (const term of searchTerms) {
        ...
        const nums = extractUSD(context);
        if (nums.length >= 2) {
          const p = makeUSD(model, displayName, nums[0], nums[1], nums.length >= 3 ? nums[2] : nums[0] * 0.1, rate, "official");
