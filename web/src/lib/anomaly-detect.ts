 ...
 * 4. 写入 alert_logs（type=anomaly）→ 通过 notification-router 发送
 *
...
async function getSettings(): Promise<{
  ...
  const map: Record<string, string> = { anomaly_threshold: "10" };
...
export async function detectAnomalies(): Promise<AnomalyCheckResult> {
  ...
  // 6. 通过 notification-router 发送通知
  if (newAnomalies.length > 0) {
    const dangerLines = newAnomalies.map(
      (a) =>
    ...
    await notifyAlert({
      type: "anomaly",
      targetId: "batch",
      message: formatAnomalyMessage(newAnomalies[0]),
      card: {
        title: `🚨 异常用量预警（${newAnomalies.length} 人）`,
        template: "red",
        elements: [
          dangerLines.join("\n\n"),
          `⏰ 检测时间：${formatBeijingDateTime()}`,
          "💡 请确认是否存在异常调用，必要时联系相关人员",
        ],
      },
    });
  }
